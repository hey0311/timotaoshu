...
router.use('', oauth(1003),  async function(req, res, next) {
    ...
    let searchSqlStart = `select id,email,bizName,fromSite,fromUrl,fromKeywords,sendStatus,reptileTime from email`
    ...
    try{
        ...
        count = (await db.query(`select count(*) from email ${sql}`))[0]["count(*)"];
    }catch(err) {
        console.log(err)
