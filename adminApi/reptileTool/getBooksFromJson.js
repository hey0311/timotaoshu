...
async function getBooksFromJson(finish) {
  ...
  for (i; i < length; i++) {
    ...
    if (tool.catalogQueue.length() > 5000) {
      ...
      log.info(
        `当前catalogQueue任务有${tool.catalogQueue.length()}条，先休息一分钟`
      );
    }
  }
  if (length == 0) {
...
async function getBook(obj, callback) {
  ...
  let reptileCommon = await reptileCommon2(reptileType, obj);
...
async function startBook(
  ...
  console.log(
    "%cgetBooksFromJson.js line:83 catalog",
    "color: #007acc;",
    catalog
  );
  let responseCount = 0,
  ...
  async function end() {
    ...
    if (responseCount == length) {
      ...
      let errorCount = (
        await db.query(
          `select count(*) from progresserror where bookId=${bookId}`
        )
      )[0]["count(*)"];
      let sql = ``;
      if (errorCount <= 0) {
