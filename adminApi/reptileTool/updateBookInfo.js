...
async function updateBookInfo_common(
  reptileType,
  book,
  bookId,
  callback,
  ...
  let reptileCommon = await reptileCommon2(reptileType, book);
  let start = 0;
  await startRp();
  async function startRp() {
    ...
    let option = {
      ...
      transform: function (body) {
        ...
        return cheerio.load(iconv.decode(body, reptileCommon.code), {
    ...
    try {
      ...
      let exists = fs.existsSync(
        path.join(__dirname, `../../books/${book.id}.png`)
      );
      ...
      await db.query(
        `update book set updateTime=date_sub("${book.updateTime}",interval 0 day),bookStatus=${book.bookStatus},bookType="${book.bookType}",imgUrl="${book.imgUrl}" where id=${bookId}`
      );
      // try{
